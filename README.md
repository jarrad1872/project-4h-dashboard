...
- `/sales` now exposes Vistaprint-ready Dustin card files under `public/sales-assets/print-hires`: six 2172x1272 proof-sheet-faithful PNGs and a ZIP pack upscaled from the approved generated card crops, so the uploaded card artwork preserves the selected creative direction in Vistaprint
- Creative source of truth: all visual assets must use `$imagegen` / `image_gen` as the finished creative path. Code may crop, upscale, package, route, display, or build clearly labeled mechanical helpers, but it must not substitute HTML/CSS/SVG/canvas renderings for generated ads, business cards, proof sheets, creator frames, hero images, thumbnails, or campaign mockups.
