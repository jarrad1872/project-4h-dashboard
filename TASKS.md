...
### Growth Command Center Rebuild (2026-03-31)
- [x] Homepage rebuilt around the live `pipe.city` pilot with launch countdown, creative pipeline, influencer pipeline, channel placeholders, and budget tracking
...
- [x] Legacy Drive CSV export removed from the active approval UI; `/api/drive-backup/export` now returns archived/gone
- [x] Launch Tier 1 trade ordering now derives from `lib/trade-utils.ts` instead of a hardcoded slug list
- [x] Removed the dead embedded AI generate panel from ad CRUD screens; `/generate` remains the active creative generation entry point
