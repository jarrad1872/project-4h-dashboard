...
- Business card print exports for Dustin must use the high-resolution production PNGs in `public/sales-assets/print-hires`, not proof-sheet crops or the legacy SVG/PNG route. The current pack is six 2172x1272 full-bleed PNGs plus `dustin-pipe-business-card-print-pack-hires.zip`; the copy, QR, contact info, and demo line are rendered at upload scale so Vistaprint does not blur the card.
...
- Dustin Field Mode uses `/api/sales/access` and the `x-sales-rep-code` header. Production writes require `PUMPCANS_DUSTIN_REP_CODE` or `PUMPCANS_SALES_REP_CODE`; localhost remains open for development.
- Rep-code access can create Dustin-owned real pipe.city rows and update Dustin-owned field-stage/next-action fields only. Bob/API token access keeps full internal write ability.
- Tracked card URLs use `/api/sales/track`, log a unique field-sales `asset_view` or `demo_call`, then land on `/sales/dustin`. The landing page only shows the demo line and rep contact; it does not send outreach, launch ads, create webhooks, move money, or change billing.
